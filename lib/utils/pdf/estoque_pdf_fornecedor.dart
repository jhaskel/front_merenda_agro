...
class EstoquePdfFornecedor extends StatefulWidget {
  /// Creates pdf with header and footer
  
  List<Estoque> list;
  Fornecedor fornecedor;

  EstoquePdfFornecedor(this.list,this.fornecedor );
  @override
  _EstoquePdfFornecedorState createState() => _EstoquePdfFornecedorState();
}

class _EstoquePdfFornecedorState extends State<EstoquePdfFornecedor> {
  _EstoquePdfFornecedorState();
  ...
  Future<void> _generatePDF() async {
    ...
    var licitacao = widget.list.map((e) => e.processo).toSet().toList();
...
//Add header to the grid
    ...
    header.cells[0].value= 'Relatório de produtos por Fornecedor';
    PdfGridRow headerFor = grid.headers[1];
    PdfGridRow headerCnpj = grid.headers[2];
    headerFor.cells[0].value= '${widget.fornecedor.nome}';
    headerCnpj.cells[0].value= 'Processo';

    for(var x in licitacao){
      print("NNNN $x");
...
//Draw the grid
    ...
    headerElement.graphics.drawString(
        '${widget.fornecedor.nome}', PdfStandardFont(PdfFontFamily.helvetica, 10),
...
//Saves the document
    ...
    js.context['filename'] = 'Fornecedor-${widget.fornecedor.alias.trim()}';
    Timer.run(() {
